...
struct RouletteWheelView: View {
    ...
    var body: some View {
        TimelineView(.animation(minimumInterval: isSpinning ? 1 / 20 : 1)) { timeline in
            ...
            wheelStack(pulse: pulse)
        }
    }

    @ViewBuilder
    private func wheelStack(pulse: Double) -> some View {
        ...
        if showsChrome {
            stack
                .background {
                    wheelFill(isHighlighted: isSpinning || jackpotFlash, pulse: pulse)
                }
                ...
                .shadow(
                    color: AppTheme.gold.opacity(isSpinning ? 0.28 + pulse * 0.12 : jackpotFlash ? 0.4 : 0.1),
                    radius: isSpinning ? 12 + pulse * 4 : jackpotFlash ? 16 : 6,
                    y: 2
                )
    ...
    @ViewBuilder
    private func wheelFill(isHighlighted: Bool, pulse: Double) -> some View {
        if showsChrome {
            ZStack {
