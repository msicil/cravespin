...
CraveRoll is an iPhone app that spins a roulette wheel to pick a restaurant near you. It pulls restaurant details from **Google Places (New)**. The winner card opens **Google Maps**, and **Reserve** opens the restaurant’s website when Google marks the place as reservable.

## Open in Xcode

1. Open `CraveRoll.xcodeproj` in Xcode (double-click or **File → Open**).
2. Select the **CraveRoll** scheme and an iPhone simulator.
3. Press **Run** (⌘R).
...
3. Create an API key restricted to your iOS bundle ID (`com.cravespin.app`) and **Places API (New)**.
   ...
   - CraveRoll sends `X-Ios-Bundle-Identifier` on each request (required for iOS-restricted keys).
...
CraveRoll reads Google Places **`reservable`** and **`websiteUri`**. When a place is reservable and has a website, the winner card shows **Reserve** (opens the restaurant site) plus **Maps**. Otherwise you get a full-width **Maps** button only.
...
`/Users/michaelsiciliano/Projects/CraveRoll`
