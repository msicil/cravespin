...
enum GoogleMapsLinker {
    ...
    /// HTTPS Maps link suitable for sharing (Messages, Mail, etc.).
    static func shareURL(for restaurant: Restaurant) -> URL? {
        if let googleMapsURL = restaurant.googleMapsURL,
           googleMapsURL.scheme == "https" || googleMapsURL.scheme == "http" {
            return googleMapsURL
        }
        return fallbackURLs(for: restaurant).first { url in
            url.scheme == "https" || url.scheme == "http"
        }
    }

    /// Plain-text summary for the system share sheet.
    static func shareMessage(for restaurant: Restaurant) -> String {
        var lines = [
            "CraveRoll picked: \(restaurant.name)",
            restaurant.address,
        ]

        if let rating = restaurant.rating {
            var ratingLine = String(format: "%.1f stars", rating)
            if let price = restaurant.priceLabel {
                ratingLine += " · \(price)"
            }
            lines.append(ratingLine)
        } else if let price = restaurant.priceLabel {
            lines.append(price)
        }

        if let isOpen = restaurant.isOpenNow {
            lines.append(isOpen ? "Open now" : "Closed")
        }

        if let url = shareURL(for: restaurant) {
